...
bool CalcularCercaOptima(PtrListaArboles Optimo, int argc, char *argv[])
{
	int MaxCombinaciones=0, NumArboles=0, min_cost=0;
	...
	for( thread = 0; thread<NumTotalThreads; thread++){
		...
		if(pthread_join(Tids[thread], (void**)&combParcial)){ //sincronitzacion de todos los threads i guardar valor de retorno a combParcial
	...
	int combOpt = combParciales[index]; //la combinacion optima sera la que se encuentre en el index devuelto por la funcion cercaCostMin
